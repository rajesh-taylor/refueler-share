...
vi.mock('../src/webhook_delivery.js', () => {
  // All helpers defined INSIDE the factory — safe from TDZ hoisting issues.
  // vi.mock is hoisted above top-level const declarations; any const captured
  // by a factory closure is in the TDZ at the time the factory runs unless
  // it is defined inside the factory itself.
...
//
// Calls deriveWhsecFromHash from the mocked webhook_delivery.js.
// The mock factory inlines the real HMAC-SHA256 + base58 derivation, so these
// tests exercise genuine cryptographic output without vi.importActual (which
// poisons the module cache and causes handleWebhookRegister to return undefined).
//
...
describe('handleWebhookRegister — method dispatch', () => {
  it('returns 405 on PATCH', async () => {
    const env = makeEnv();
    const req = new Request('https://api.share.refueler.io/api/v1/webhook/register', { method: 'PATCH', body: '{}', headers: { 'Content-Type': 'application/json' } });
    const resp = await handleWebhookRegister(req, env);
  ...
  it('returns 405 on PUT', async () => {
    const env = makeEnv();
    const req = new Request('https://api.share.refueler.io/api/v1/webhook/register', { method: 'PUT', body: '{}', headers: { 'Content-Type': 'application/json' } });
    const resp = await handleWebhookRegister(req, env);
    expect(resp.status).toBe(405);
  });
});
