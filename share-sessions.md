...
2. **Share-DAD-2** (Sonnet, small, frontend — `frontend/download.js` + `ship-frontend.sh`) — the three DAD UX items above. Open decision first: removing the "I've saved it" step means deleting as soon as the download finishes; if the browser's save silently fails the file is gone. Pre- or post-Berlin at Rajesh's call.
